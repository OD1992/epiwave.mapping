...
buffer.image <- raster("/mnt/s3/stratification/CountryFolders/Nigeria/Common files/Population/u5_grid3_1km_new.tif")

buffer.image <- crop(buffer.image, raster(paste0(dir_path, "/Common files/Covariates/1km/EVI_v6.2018.Annual.mean.1km.Data.tif")))
                     
population   <- getValues(buffer.image)
# population   <- getValues(buffer.image)*0.16

# Create an index vector of each pixel, along with their coordinates
valid.pix     <- which(!is.na(population) & population>0)
im.coords     <- coordinates(buffer.image)[valid.pix,]
country.index <- numeric(length(valid.pix))+NA

library(raster)
library(sf)
shape <- st_read(file.path(dir_path,"Common files/Shapefiles/NGA_LGAs.shp"))  # Ensure this returns a SpatialPolygonsDataFrame
# Convert sf object to SpatialPolygonsDataFrame
shape_sp <- as(shape, "Spatial")  # Converts to SpatialPolygonsDataFrame

# Extract values using raster::extract()
values <- extract(buffer.image, shape_sp)

# Update country.index
country.index[!is.na(values[valid.pix])] <- 0

library(tidyverse)
country.shp <- st_read(file.path(dir_path,"Common files/Shapefiles/NGA_LGAs.shp")) %>% mutate(
...
# Create raster of country-assigned pixels. 
country.image         <- buffer.image
buffer                <- numeric(length(buffer.image))+NA
buffer[valid.pix]     <- country.index
values(country.image) <- buffer
plot(country.image)

# Determine valid pixels (total and in-country-only), and their coordinates 
Nvalid.pix        <- length(valid.pix)
in.country.pix    <- which(country.index==0)
Ncountry.pix      <- length(in.country.pix)
valid.coords      <- im.coords
in.country.coords <- im.coords[in.country.pix,]
...
covariates_list <- paste(raster_dir, covariate.filelist, sep = "/")
...
ras_names <- c(
  ...
  "Tree fraction",
  "TWI")
names(covariates_list) <- ras_names

# Mask and scaled raster covariates_list
covariates_stack <- covariates_list%>% 
  #map(raster) %>% 
  stack() %>% 
  mask(country.shp) %>% 
  scale()
...
covariates_stack[covariates_stack > 5]  <- 5
covariates_stack[covariates_stack < -5] <- -5

# Convert to SpatRaster because it was a RasterBrick
library(terra)
covariates_stack <-rast(covariates_stack)
```
...
# library(raster)
# # Read in population data and construct reference image =====
# 
# pop_raster_u5 <- raster("/mnt/s3/stratification/CountryFolders/Nigeria/Common files/Population/u5_grid3_1km_new.tif")
# 
# any(is.na(values(pop_raster_u5)))  # Returns TRUE if there are NA values
# plot(is.na(pop_raster_u5), main = "NA Distribution in Population Raster")
# 
# # find an index to all the cells that have data
# pop_raster_u5 <- rast(pop_raster_u5)
# not_na_idx <- which(!is.nan(values(pop_raster_u5)))
# 
# # Get all raster values
# pop_values <- values(pop_raster_u5)
# 
# # Mask: Keep only values in `not_na_idx`, set others to NA
# pop_values[-not_na_idx] <- NA  
# 
# # Assign masked values back to the raster
# pop_raster_masked <- pop_raster_u5  # Copy the original raster structure
# values(pop_raster_masked) <- pop_values  # Apply the masked values
# 
# # Plot the masked raster
# plot(pop_raster_masked, main = "Masked Population Raster")
```
...
# library(terra)
# 
# pop_raster_u5 <- pop_raster_masked
# # Match extends
# pop_nga <- extend(pop_raster_u5, covariates_stack[[1]])
# 
# # Align resolution
# pop_nga <- resample(pop_nga, covariates_stack[[1]], method = "bilinear")  # Use "near" for categorical data
# 
# # # crop and mask to NGA
# pop <- crop(pop_nga, covariates_stack[[1]]) 
# pop <- mask(pop, covariates_stack[[1]])  
```
...
#st_crs(pop)
#st_crs(covariates_stack[[1]])
covariates_raster <- terra::project(covariates_stack, new_crs)
#pop_raster <- terra::project(pop, new_crs)
```
...
pop_raster_df <- as.data.frame(buffer.image, xy = TRUE)
...
itn_stack <-itn_stack %>%
  resample(buffer.image) %>%  #added resample to align the extents
  crop(buffer.image) %>% 
  mask(buffer.image)
...
#bioclim_raster <- terra::aggregate(covariates_raster, 20)
#pop_raster <- terra::aggregate(pop_raster, 20)
...
# covariates_raster <- cov_stack_nga
# # set time periods
# start_year <- 2021
# n_years <- 3
# 
...
#   n_prev_surveys = 15,
#   case_missingness = 0.3, # assume some fraction of case counts are missing
#   prev_survey_time = prev_survey_time)
```
