...
Next we prepare our covariates. We will lower the spatial resolution a little to make the example run faster. This is a good idea when initially setting up and debugging a model, but you'll likely want to increase the spatial resolution later.

This package is under development by Mauricio and Juniper (MAp)
```{r}
# WELCOME TO THE malariaAtlasCovariates R PACKAGE DEMO 2
# Note that this package is under development!
# First: Install from GitLab (currently everyone can use the following token)
devtools::install_gitlab("malaria-atlas-project/data-eng/covariates/malariaatlascovariates", auth_token = "glpat-HYbDsA7b-Ds94JQ1nZP7")
# Once installed, the optimal workflow is to:
  # 1) Review available covariates
...
## Let's get started! ##############################################################################################################################################################
# Load libraries
library(malariaAtlasCovariates)
library(terra)
library(dplyr)
library(raster)
library(viridis)
...
#?malariaAtlasCovariates

# Step 1: Let's check out the covariates we can load
covs.list_covariates()

# Step 2: List variable options for a variable
#cov.list_available_variables("lst_v061")

# Step 3: Send off a single covariate query for Nigeria
# query_result_day <- cov.query(    cov.cov("lst_v061", type = c("day", "night")),
#                                   temporal_resolution = "Annual",
#                                   spatial_resolution = "1km",   # 1km/5km or sometimes 500m etc
#                                   temporal_summary = "mean",
#                                   spatial_summary = "Data"  # Change this from "mean" to "Data"
#                                   ) %>%
#   cov.filter_date("2021", "2023") %>%     #To and from
#   cov.clip(geo.countries(version="mapadmin_0_2022", "NGA")) %>% # Note the version name!
#   cov.load_rasters_as_stack() # Load in rasters/or cov.load_rasters_as_stack() function to load rasterstack object

# Getting my dynamic covariates
# test, not a raster stack
lst_day_1km <- cov.query(    cov.cov("lst_v061", type = c("day")),
  ...
  cov.filter_date("2021", "2023") %>%     #To and from
  cov.clip(geo.countries(version="mapadmin_0_2022", "NGA")) %>% # Note the version name!
  cov.load_rasters() # Load in rasters/or cov.load_rasters_as_stack() function to load rasterstack object
#lst_day_1km %>%cov.pull_first_raster()
...
```{r TS}
library(ggplot2)
all_s_ts <- raster("/mnt/s3/stratification/CountryFolders/Nigeria/Projects/1.0_HBHI_risk_mapping/04 Models/jan2023-update/treatment-seeking/2021_ts_dhis2_1km.tif")

all_s_ts <- rast(all_s_ts)  # Convert if all_s_ts is a RasterLayer
  
# Step 2: Make sure both rasters have the same CRS (coordinate reference system)
if (crs(all_s_ts) != crs(pop_u5)) {
  all_s_ts <- project(all_s_ts, crs(pop_u5))  # Reproject all_s_ts to match buffer.image's CRS
}

# Step 3: Crop or Extend
# If you want to crop all_s_ts to the extent of buffer.image
all_s_ts <- crop(all_s_ts, pop_u5)  # Crop to the extent of buffer.image

# If you want to extend all_s_ts to match the extent of buffer.image
all_s_ts <- extend(all_s_ts, ext(pop_u5))  # Extend to the extent of buffer.image

shape$alls_ts <- exactextractr::exact_extract(all_s_ts,shape, 'mean')
ggplot(shape)+
...
```{r treatment_seeking_matrix}
if (!prod(dim(all_s_ts)==dim(pop_u5))) 
{warning("Mismatched dimensions!\n")
  all_s_ts <- resample(all_s_ts, pop_u5)
  plot(all_s_ts)}
all_s_ts <- values(all_s_ts)[valid.pix]

nas <- which(is.na(all_s_ts))
if (length(nas)>0) {
  validp  <- which(!(is.na(all_s_ts) | all_s_ts==-Inf | all_s_ts==Inf))
  vcoords <- valid.coords[validp,]
  for (i in nas) {
    nearestv <- which.min(
      (vcoords[,1]-valid.coords[i,1])^2+(vcoords[,2]-valid.coords[i,2])^2)
    all_s_ts[i] <- all_s_ts[validp[nearestv]]
  }
}

infs <- which(all_s_ts==Inf)
if (length(infs)>0) {
  validp  <- which(!(all_s_ts==-Inf | all_s_ts==Inf))
  vcoords <- valid.coords[validp,]
  for (i in infs) {
    nearestv <- which.min(
      (vcoords[,1]-valid.coords[i,1])^2+(vcoords[,2]-valid.coords[i,2])^2)
...
neginfs <- which(all_s_ts==-Inf)
if (length(neginfs)>0) {
  validp  <- which(!(all_s_ts==-Inf))
  vcoords <- valid.coords[validp,]
  for (i in neginfs) {
    nearestv <- which.min(
      (vcoords[,1]-valid.coords[i,1])^2+(vcoords[,2]-valid.coords[i,2])^2)
...
library(raster)
library(terra)
# Loop through each month and process
ITN_temporal_cov <- matrix(0,nrow=length(valid.pix),ncol=12 )
for (month_idx in 1:12) {
    # Loading the current month raster
    cov_current <- raster(paste0("/mnt/s3/stratification/CountryFolders/Nigeria/Common files/Covariates/1km/Monthly/ITN/20210531_v1//ITN_use_1km_monthly_", month_idx, ".tif")) 
    cov_current <- rast(cov_current)  # Convert if cov_current is a RasterLayer
    if (crs(cov_current) != crs(pop_u5)) {
      cov_current <- project(cov_current, crs(pop_u5))  
    }
    cov_current <- crop(cov_current, pop_u5)  
    cov_current <- extend(cov_current, ext(pop_u5))  # Extend to the extent of buffer.image
    # Ensure matching dimensions
    if (!prod(dim(cov_current) == dim(pop_u5))) {
        stop("Mismatched dimensions!\n")
      cov_current <- resample(cov_current, pop_u5)
    }
    # Extract valid pixels
    cov_current_values <- values(cov_current)[valid.pix]

    # Handle NA, Inf, and -Inf values
    na_indices <- which(is.na(cov_current_values))
    inf_indices <- which(cov_current_values == Inf)
    neginf_indices <- which(cov_current_values == -Inf)

    # Replace missing values (NA, Inf, -Inf) by nearest valid value
    for (indices in list(na_indices, inf_indices, neginf_indices)) {
        if (length(indices) > 0) {
            validp <- which(!(cov_current_values == -Inf | cov_current_values == Inf | is.na(cov_current_values)))
            vcoords <- valid.coords[validp,]
            for (i in indices) {
                nearestv <- which.min((vcoords[,1] - valid.coords[i, 1])^2 + (vcoords[,2] - valid.coords[i, 2])^2)
                cov_current_values[i] <- cov_current_values[validp[nearestv]]
            }
        }
    }

    # Store the processed data
    ITN_temporal_cov[, month_idx] <- cov_current_values
    
    # Print the month
    cat(month.name[month_idx], " ")
...
ggplot(aes(
   ...
   facet_wrap(~year, nrow = 3) +  # Set the number of rows to 2 (or change this value as needed)
 ...
 ggsave("~/epiwave.mapping/clinical_cases_timeseries_2021_2023.png")
```
...
prev_inc_plot <- tibble(prevalence = seq(0, 0.5, length.out = 100)) %>%
   mutate(
     clinical_incidence = g(prevalence)
   )
# 
 ggplot(
   aes(x = prevalence,
...
Prevalence observed timeseries
```{r vis_data_6}
q_max_days <- 30
q_star <- sum(q_daily(seq(0, q_max_days)))
...
beta <- normal(0, 1, dim = ncol(covariates))

beta_itn <- normal(0, 1, dim = ncol(ITN_temporal_cov), truncation = c(0,Inf))
...
# matrix-multiply with beta, and add alpha, to get the spatial fixed effects, ITN use effects
fixef <- alpha + X %*% beta - ITN_temporal_cov %*% beta_itn
...
clinical_case_incidence <- g(q_star * infection_incidence*all_s_ts)
