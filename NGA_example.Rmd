...
population <- values(pop_u5)[valid_cells]
raster_coords <- terra::xyFromCell(pop_u5, valid_cells)
...
#              
# im.coords     <- xyFromCell(raster(pop_u5), valid.pix)
...
Static and dynamic covariates combined
```{r}
...
if (crs(all_s_ts) != crs(pop_u5)) {
  all_s_ts <- terra::project(all_s_ts, crs(pop_u5))  # Reproject all_s_ts to match buffer.image's CRS
}
...
for (month_idx in 1:12) {
    ...
    cov_current <- extend(cov_current, ext(pop_u5))  
    # Ensure matching dimensions
    # if (!all(dim(cov_current) == dim(pop_u5))) {
    #     stop("Mismatched dimensions!\n")
    #   cov_current <- resample(cov_current, pop_u5)
    # }
    if (!all(dim(cov_current) == dim(pop_u5))) {
    message("Resampling due to dimension mismatch: ", paste(dim(cov_current), collapse = "x"), " vs ", paste(dim(pop_u5), collapse = "x"))
    cov_current <- resample(cov_current, pop_u5)
}
...
beta <- normal(0, 1, dim = ncol(covariates_static))
...
eta <- sweep(epsilon, 1, fixef[1:nrow(epsilon)], FUN = "+")
...
cases_expected <- reported_clinical_cases[cases_extract_index]#reported_clinical_cases_hf[cases_extract_index]


y_obs <- cases_observed
...
m <- model(alpha, beta, beta_itn, phi, theta, sigma)
