...
clinical_cases_pixel_plot <- ggplot() +
    geom_spatraster(data = 365 * clinical_cases_pixel_post_raster[[times_plot]]) +
    scale_fill_distiller(
      name = "Cases",
      palette = "YlOrRd",
      direction = 1,
...
# get true values
param_names <- c("alpha", "beta", "phi", "theta", "sigma")
...
post_means <- unlist(lapply(param_post, colMeans))
prior_means <- unlist(lapply(param_prior, colMeans))

round(cbind(truth = param_truth,
      post_mean = post_means,
      prior_mean = prior_means), 2)
```
