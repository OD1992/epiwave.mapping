...
# aggregate rasters
bioclim_kenya_lores <- terra::aggregate(bioclim_kenya, 20)
pop_kenya_lores <- terra::aggregate(pop_kenya, 20)
...
```{r run_mcmc}
# list only the quantitites/parameters we want to trace the posteriors for in
# the first instance. We can always recompute other quantities later, so keep
# this to the hyperparameters.
m <- model(
  alpha, beta,
  r,
  phi, theta, sigma
...
# load the latent white noise variable for the space time process into this
# environment so we can set initial values for them
v <- attr(epsilon, "v")
...
# sim <- calculate(prevalence_estimates,
#                  cases_expected,
#                  nsim = 1,
#                  values = inits[[1]])
# summary(sim$prevalence_estimates)
# summary(sim$cases_expected)
